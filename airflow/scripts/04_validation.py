...
def run_validation(index_name="anime"):
    # ... tout ton code d'indexation actuel ...
    BASE_DIR = Path(__file__).resolve().parent.parent
    OUTPUT_DIR = BASE_DIR / "output"
    OUTPUT_DIR.mkdir(exist_ok=True)

    INPUT_FILE = OUTPUT_DIR / "anime_gold.csv"
    VALIDATED_CSV = OUTPUT_DIR / "anime_gold_validated.csv"
    VALIDATED_JSON = OUTPUT_DIR / "anime_gold.json"
    ...
    success_rate = (total_pass / total_checks * 100) if total_checks else 0

    print(f"""
    {C.BOLD}  Assertions passées  : {C.G}{total_pass}{C.END}
    {C.BOLD}  Assertions échouées : {C.F if total_fail > 0 else C.G}{total_fail}{C.END}
    {C.BOLD}  Total               : {total_checks}{C.END}
    {C.BOLD}  Taux de réussite    : {success_rate:.0f}%{C.END}
    """)
    ...
    📄 {str(VALIDATED_CSV):45s} (dataset final CSV)
    📄 {str(VALIDATED_JSON):45s} (prêt pour Elasticsearch)
    📄 {str(REPORT_FILE):45s} (rapport de validation)
