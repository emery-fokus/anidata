...
# Avant : ton code était juste "en vrac" dans le fichier
# Maintenant :
def run_normalisation(index_name="anime"):
    # ============================================
    # CONFIG
    # ============================================
    BASE_DIR = Path(__file__).resolve().parent.parent
    OUTPUT_DIR = BASE_DIR / "output"
    INPUT_FILE = OUTPUT_DIR / "anime_cleaned.csv"
    GOLD_FILE = OUTPUT_DIR / "anime_gold.csv"
    ...
    # ============================================
    # FEATURE 5B — MEILLEUR ANIME PAR DÉCENNIE
    # ============================================
    step("Feature 5B : Meilleur anime par décennie")

    if "decade" in df.columns and "score" in df.columns:
        name_col = "name" if "name" in df.columns else None
        ranking_col = "weighted_score" if "weighted_score" in df.columns else "score"
        ...
        df["is_best_of_decade"] = False
        df["best_of_decade_label"] = np.nan

        valid_best = df[df["decade"].notna() & df["score"].notna()].copy()
        if name_col and not valid_best.empty:
            sort_cols = ["decade", ranking_col]
            ascending = [True, False]
            if "members" in valid_best.columns:
                sort_cols.append("members")
            ...
            for idx, row in best_by_decade.iterrows():
                decade = int(row["decade"])
                anime_name = str(row[name_col])
                df.loc[idx, "is_best_of_decade"] = True
                df.loc[idx, "best_of_decade_label"] = f"{decade}s - {anime_name}"

            ok(f"Colonnes 'is_best_of_decade' et 'best_of_decade_label' créées ({len(best_by_decade)} décennies)")
            print("\n  Meilleurs animes par décennie :")
            for _, row in best_by_decade.sort_values("decade").iterrows():
                decade = int(row["decade"])
                anime_name = row[name_col]
                anime_score = row["score"]
                print(f"    {decade}s : {anime_name} (score {anime_score:.2f})")
        else:
            warn("Impossible de déterminer le meilleur anime par décennie")
    else:
        warn("Colonnes 'decade' ou 'score' introuvables — feature ignorée")
    ...
    if genre_col:
        ...
        df[genre_col] = df[genre_col].apply(normalize_genres)
